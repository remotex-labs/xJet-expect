...
- **Report a bug**: open a [bug report](https://github.com/remotex-labs/xJet-expect/issues/new?template=bug_report.md) with a minimal reproduction.
- **Request a feature**: open a [feature request](https://github.com/remotex-labs/xJet-expect/issues/new?template=feature_request.md) describing the use case.
- **Send a pull request**: fix a bug, add a feature, or improve the docs.

Search [existing issues](https://github.com/remotex-labs/xJet-expect/issues) first to avoid duplicates.

## Development setup

xJet-Expect uses [pnpm](https://pnpm.io) and requires Node.js 20 or later.

```bash
git clone https://github.com/remotex-labs/xJet-expect.git
cd xJet-expect
pnpm install
```
...
```text
matchers: Add toBeWithinRange matcher
docs: Rewrite the mock guide
providers: Fix asymmetric equality for Map
```
...
Tests use **xJet**. Place a `*.spec.ts` file next to the code it covers, and assert both the passing and failing
paths of the matcher.

```ts
import { xExpect } from '@remotex-labs/xjet-expect';

describe('toEqual', () => {
    test('matches structurally equal objects', () => {
        xExpect({ id: 42 }).toEqual({ id: 42 });
    });
});
```

Cover edge cases: negated matchers (`.not`), asymmetric matchers, async `resolves`/`rejects`, and invalid input
that should throw a clear assertion error.
...
xJet-Expect follows [Semantic Versioning](https://semver.org/): MAJOR for incompatible API changes, MINOR for backward-compatible features, and PATCH for backward-compatible fixes.
