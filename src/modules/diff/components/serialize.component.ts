...
export function serializePrimitive(element: unknown): string | null {
    ...
    if (typeof element === 'number') return `${ Number.isFinite(element) ? element : 'NaN' }`;
...
export function serializeMap(map: Map<unknown, unknown>, lines: Array<string>, indent: string): void {
    if (map.size === 0) {
        lines.push('Map {}');
    ...
    entries.forEach(([ key, val ], i) => {
        ...
        serializeValue(val, subLines, indent);
...
export function serializeArray(arr: Array<unknown>, lines: Array<string>, indent: string): void {
    if (arr.length === 0) {
        lines.push('Array []');
    ...
    arr.forEach((item, i) => {
        const subLines: Array<string> = [];
        serializeValue(item, subLines, indent);
...
export function serializeSet(set: Set<unknown>, lines: Array<string>, indent: string): void {
    if (set.size === 0) {
        lines.push('Set {}');
...
export function serializeObject(obj: object, lines: Array<string>, indent: string): void {
...
export function serializeValue(value: unknown, lines: Array<string>, indent: string = INDENT): void {
    ...
    if (value instanceof Map) {
        serializeMap(value, lines, indent);
    } else if (Array.isArray(value)) {
        serializeArray(value, lines, indent);
    } else if (value instanceof Set) {
        serializeSet(value, lines, indent);
    } else if (value instanceof Buffer) {
        serializeBuffer(value, lines, indent);
    } else if (ArrayBuffer.isView(value) && !(value instanceof DataView)) {
        serializeTypedArray(value, lines, indent);
    } else {
        serializeObject(value as object, lines, indent);
    }
...
export function serialize(element: unknown, indent: string = INDENT): Array<string> {
    ...
    serializeValue(element, lines, indent);
